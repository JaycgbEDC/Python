...
Base_api = "https://match.yuanrenxue.cn/api/match/4"
...
pattern = re.compile(r'left:(.*)px')


def every_page(i: int):
    res = 0
    params = {
        "page": i
    }
    ...
    response = requests.get(url = Base_api, params = params, proxies=proxies)
    html = response.json()["info"]
    encrypt = hashlib.md5((base64.b64encode((response.json()["key"] + response.json()["value"]).encode("utf-8")).decode("utf-8").replace('=', '')).encode()).hexdigest()
    initial = etree.HTML(html)
    td_list = initial.xpath("//td")
    for td in td_list:
        num_list = ['', '', '', '']
        index = 0
        img_list = td.xpath("./img")
        for img in img_list:
            display = img.xpath("./@class")
            if (encrypt in display[0]):
                continue
            src = img.xpath("./@src")[0]
            offset = pattern.findall(img.xpath("./@style")[0])[0]
            num_list[index + int(float(offset) / 11.5)] = digits[src]
            index += 1
        print(num_list)
